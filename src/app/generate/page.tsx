...
export default function GeneratePage() {
  ...
  const handleRecipeQuery = async () => {
    if (!recipeQuery.trim()) {
      setRecipeError('Enter a recipe query');
      return;
    }
    ...
    try {
      ...
      if (data.success && data.html) {
        // Store content in sessionStorage to avoid URL size limits
        console.log('Storing content in sessionStorage, length:', data.html.length);
        sessionStorage.setItem('generatedContent', data.html);
        sessionStorage.setItem('generatedTitle', title || 'Recipe Article');
        console.log('Content stored, navigating to editor');
        router.push('/editor');
      } else {
      ...
      setRecipeError(error instanceof Error ? error.message : 'Failed to generate recipe content');
    } finally {
          ...
          {articleType === 'Recipe article' ? (
            <div>
              ...
              <div className="space-y-3">
                ...
                />
                <button
                  onClick={handleRecipeQuery}
                  disabled={recipeLoading || !recipeQuery.trim()}
                  className="w-full bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-4 rounded shadow disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {recipeLoading ? 'Generating Recipe...' : 'Create Recipe Article'}
                </button>
                {recipeError && (
